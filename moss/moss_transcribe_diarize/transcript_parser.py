...
def parse_transcript(text: str, **parser_kwargs) -> list[TranscriptSegment]:
    # Some outputs use the end of an empty segment as the next start:
    # [0.00][S01][0.06][S01]word[0.12]. Remove only the empty prefix,
    # retaining the explicitly generated timestamp/speaker for the real text.
    empty_shared_boundary = r"\[\d+(?:\.\d+)?\]\s*\[S\d+\]\s*(?=\[\d+(?:\.\d+)?\]\s*\[S\d+\])"
    text = re.sub(empty_shared_boundary, "", text)
    parser = TranscriptStreamParser(**parser_kwargs)
