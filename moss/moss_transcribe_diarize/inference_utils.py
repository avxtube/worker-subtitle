...
DEFAULT_PROMPT = Path(__file__).with_name("default_prompt.txt").read_text(encoding="utf-8").strip()
...
def generate_transcription(
    ...
    *,
    ...
    if timed_output:
        from .timed_decoding import TimedLogitsProcessor
        generate_kwargs["logits_processor"] = [TimedLogitsProcessor(processor.tokenizer, allow_initial_silence=timed_output == "auto")]
